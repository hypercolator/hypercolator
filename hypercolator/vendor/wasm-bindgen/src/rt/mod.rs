...
pub fn js_panic(err: JsValue) {
    // panic_any is only meaningful on wasm32 where JsValue can be caught.
    // On other targets (including the Solana SBF target) the SBF std does
    // not provide panic_any, so we fall back to core::panic! universally.
    #[cfg(all(feature = "std", not(target_feature = "atomics"), target_arch = "wasm32"))]
    ::std::panic::panic_any(err);
    #[cfg(not(all(feature = "std", not(target_feature = "atomics"), target_arch = "wasm32")))]
    ::core::panic!("{:?}", err);
}
